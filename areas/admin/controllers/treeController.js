...
module.exports = {
    ...
    get_buList: [sessionFilter, function (req, res) {
        var id = req.query.id || '0';
        treeService.getParents(id, 2, req.session.admin_login_info).then(function (ret) {
            res.jsonWrap(ret);
        }).catch(function (e) {
            console.log(e);
            res.jsonWrap(e, 1, '操作失败');
    ...
    // 修改、添加区域
    post_saveBu : [sessionFilter, function(req, res) {
        var data = req.body;
        data.Type = 2;
        treeService.saveNode(data,req.session.admin_login_info).then(function (ret) {
            res.jsonWrap(ret);
    ...
    get_deleteBu : [sessionFilter, function(req, res){
        var Id = req.query.Id;
        treeService.deleteNode(Id).then(function (ret) {
            res.jsonWrap(ret);
        }).catch(function (e) {
