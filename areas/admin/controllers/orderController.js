...
module.exports = {
    get_query : [sessionFilter,function(req, res) {
        ...
        orderService.queryOrder(pageLimit.page,pageLimit.limit,data,req.session.admin_login_info).then(function(data){
            res.jsonWrap(data);
        }).catch(function(err){
            console.log(err);
    ...
    get_turnOver : [sessionFilter, function (req, res) {
       var data = req.query;
        orderService.queryOrderTurnOver(data, req.session.admin_login_info).then(function (data) {
            res.jsonWrap(data);
        }).catch(function (err) {
            console.log(err);
            res.jsonWrap(null, 1, err);
    ...
    get_goodsInOrder : [sessionFilter, function (req,res) {
        var pageLimit = pageValidate.requestFilter(req);
        var orderid = req.query.orderid;
        orderService.queryGoods4Order(pageLimit.page,pageLimit.limit,orderid,req.session.admin_login_info).then(function (data) {
            res.jsonWrap(data);
        }).catch(function (err) {
            res.jsonWrap(null, 1, err);
        })
