...
module.exports = {
    ...
    get_queryAll: [sessionFilter, function (req, res) {
        ...
        goodsService.goodsList(page, limit, data, req.session.admin_login_info).then(function (data) {
            res.jsonWrap(data);
        }).catch(function (msg) {
            console.log(msg);
    ...
    post_save: [sessionFilter, function (req, res) {
        ...
        if(!data.Id){
            goodsService.checkGoodsCode(data["GoodsCode"]).then(function(isRepeat){
                if(isRepeat){
                    res.jsonWrap(null, 1, "该商品Code已存在!");
                    return;
                }
