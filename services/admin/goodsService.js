...
var goodsService = {
    // 获取我的商品列表
    goodsList: function (page, limit, data, session) {
        return new Promise(function (resolve, reject) {
            var offset = (page - 1) * limit;
            var IsEnable = parseInt(data.IsEnable || -1);
            ...
            if (data.name) {
                whereCase.GoodsShortTitle = {'$like': '%' + data.name + '%'};
            }
    ...
    checkGoodsCode: async(function(goodsCode){
        var goodsTemp = await(db.goods.findOne({where: {GoodsCode: goodsCode}}));
        if(goodsTemp){
            return true;
        }
        return false;
    ...
    saveLeaderGoods: async(function (goods, session) {
        ...
        if (goodsInfo) {
            ...
            for (var i = 0; i < goodsArr.length; i++) {
                ...
                goodsArr[i].GoodsSupplyPrice = goodsArr[i].GoodsSalePrice;
                goodsArr[i].GoodsSalePrice = goodsArr[i].GoodsOriginalPrice;
